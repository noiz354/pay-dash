...
// each test starts from the seeded baseline. Wave 7C: every read/write takes
// the demo tenant explicitly — the legacy suite runs single-tenant.
...
describe("listCustomers", () => {
  it("derives the directory from the ledger and keeps the prototype seeds", async () => {
    const { rows, total } = await listCustomers(ctx, { pageSize: 100 });
  ...
  it("filters by free-text query across name and email", async () => {
    const { rows, isFiltered } = await listCustomers(ctx, { q: "stark", pageSize: 100 });
  ...
  it("filters by status", async () => {
    const { rows } = await listCustomers(ctx, { status: "REVIEW", pageSize: 100 });
    expect(rows.every((c) => c.status === "REVIEW")).toBe(true);
  });

  it("sorts by lifetime value and by name", async () => {
    const byLtv = await listCustomers(ctx, { sort: "ltv", pageSize: 100 });
    const values = byLtv.rows.map((c) => c.lifetimeValue);
    expect([...values].sort((a, b) => b - a)).toEqual(values);

    const byName = await listCustomers(ctx, { sort: "name", pageSize: 100 });
  ...
  it("paginates and clamps out-of-range pages", async () => {
    ...
    const first = await listCustomers(ctx, { page: 1, pageSize: 5 });
    expect(first.rows.length).toBeLessThanOrEqual(5);
    expect(first.pageCount).toBe(Math.ceil(first.total / 5));

    const far = await listCustomers(ctx, { page: 999, pageSize: 5 });
...
describe("createCustomer / updateCustomer", () => {
  it("creates a customer retrievable by id and by email", async () => {
    const created = await createCustomer(ctx, { name: "Wayne Enterprises", email: "Bruce@Wayne.com" });
    expect(created.id).toBe(customerIdFromEmail("bruce@wayne.com"));
    expect(created.status).toBe("NEW");
    expect(await getCustomer(ctx, created.id)).not.toBeNull();
    expect(await getCustomer(ctx, "bruce@wayne.com")).not.toBeNull();
  });

  it("rejects duplicate emails", async () => {
    await createCustomer(ctx, { name: "Wayne", email: "bruce@wayne.com" });
    await expect(createCustomer(ctx, { name: "Wayne again", email: "bruce@wayne.com" })).rejects.toThrow(
      /already exists/i
    );
  });

  it("applies overrides without dropping the underlying record", async () => {
    const created = await createCustomer(ctx, { name: "Wayne", email: "bruce@wayne.com" });
    const updated = await updateCustomer(ctx, { id: created.id, name: "Wayne Enterprises", status: "ACTIVE" });
  ...
  it("archiving is a status transition, never a delete", async () => {
    const created = await createCustomer(ctx, { name: "Wayne", email: "bruce@wayne.com" });
    await updateCustomer(ctx, { id: created.id, status: "BLOCKED" });
    const after = await getCustomer(ctx, created.id);
    expect(after).not.toBeNull();
    expect(after?.status).toBe("BLOCKED");
  });

  it("returns null when updating an unknown customer", async () => {
    expect(await updateCustomer(ctx, { id: "cus_nope", name: "Ghost" })).toBeNull();
  });
});

describe("customer <-> ledger consistency", () => {
  it("payments listed for a customer all belong to that customer", async () => {
    const { rows } = await listCustomers(ctx, { sort: "ltv", pageSize: 100 });
    const withPayments = rows.find((c) => c.paymentCount > 0);
    expect(withPayments).toBeDefined();
    const txns = await getCustomerTransactions(ctx, withPayments!.email);
  ...
  it("reports metrics that add up", async () => {
    const m = await getCustomerMetrics(ctx);
    const { total } = await listCustomers(ctx, { pageSize: 100 });
...
describe("customersToCsv", () => {
  it("emits a header plus one row per customer", async () => {
    const { rows } = await listCustomers(ctx, { pageSize: 5 });
