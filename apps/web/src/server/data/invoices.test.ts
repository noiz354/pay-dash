...
// Wave 7D: invoices are tenant-scoped. This suite is the product-behaviour suite
// (prototype statements, derivation, line-item arithmetic, CSV shape), so it runs
// single-tenant against the demo partition — the isolation pairs live in
// `invoices.tenant-isolation.test.ts`, the money-mutation order pin in B-13
// there, and the contract ratchet in `billing-structural.test.ts`.
...
describe("listInvoices", () => {
  it("returns the seeded prototype invoices plus ledger-derived months", async () => {
    const { rows, total } = await listInvoices(demo, { pageSize: 100 });
  ...
  it("filters by status and by free text", async () => {
    const paid = await listInvoices(demo, { status: "PAID", pageSize: 100 });
    expect(paid.rows.every((r) => r.status === "PAID")).toBe(true);
    expect(paid.isFiltered).toBe(true);

    const search = await listInvoices(demo, { q: "4421", pageSize: 100 });
  ...
  it("sorts by amount and by due date", async () => {
    const byAmount = await listInvoices(demo, { sort: "amount", pageSize: 100 });
    const amounts = byAmount.rows.map((r) => r.amount);
    expect([...amounts].sort((a, b) => b - a)).toEqual(amounts);

    const byDue = await listInvoices(demo, { sort: "due", pageSize: 100 });
    const dues = byDue.rows.map((r) => r.dueAt);
    expect([...dues].sort()).toEqual(dues);
  });

  it("clamps out-of-range pages", async () => {
    const far = await listInvoices(demo, { page: 999, pageSize: 5 });
...
describe("getInvoice / line items / timeline", () => {
  it("resolves an invoice by id and by number", async () => {
    expect(await getInvoice(demo, "INV-2023-08-4421")).not.toBeNull();
    expect(await getInvoice(demo, "nope")).toBeNull();
  });

  it("line items always add up to the invoice total", async () => {
    const { rows } = await listInvoices(demo, { pageSize: 100 });
    for (const inv of rows.slice(0, 5)) {
      const items = await getInvoiceLineItems(demo, inv.id);
  ...
  it("derived invoices only bill settled transactions inside their period", async () => {
    ...
    const txns = await getInvoiceTransactions(demo, derived!.id);
  ...
  it("builds a chronological timeline that ends in a payment outcome", async () => {
    const events = await getInvoiceTimeline(demo, "INV-2023-08-4421");
...
describe("payInvoice", () => {
  it("settles a payable invoice and returns a reference", async () => {
    const result = await payInvoice(demo, "INV-2023-09-5102", "Corporate card — Visa •••• 4242");
  ...
  it("refuses to double-pay", async () => {
    await payInvoice(demo, "INV-2023-09-5102", "Bank transfer — Mandiri");
    await expect(payInvoice(demo, "INV-2023-09-5102", "Bank transfer — Mandiri")).rejects.toThrow(/already paid/i);
  });

  it("returns null for an unknown invoice", async () => {
    expect(await payInvoice(demo, "INV-NOPE", "Bank transfer — Mandiri")).toBeNull();
  });

  it("reduces the outstanding balance in the summary", async () => {
    const before = await getBillingSummary(demo);
    await payInvoice(demo, "INV-2023-07-3990", "Bank transfer — Mandiri");
    const after = await getBillingSummary(demo);
...
describe("exports", () => {
  it("invoicesToCsv emits a header plus one row per invoice", async () => {
    const { rows } = await listInvoices(demo, { pageSize: 5 });
  ...
  it("invoiceStatementCsv includes the header block and the line items", async () => {
    const csv = await invoiceStatementCsv(demo, "INV-2023-08-4421");
    expect(csv).toBeTruthy();
    expect(csv!).toContain("invoice");
    expect(csv!).toContain("line_item");
    expect(await invoiceStatementCsv(demo, "INV-NOPE")).toBeNull();
  });
});
