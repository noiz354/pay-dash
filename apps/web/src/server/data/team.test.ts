...
describe("team store (ADR-0022)", () => {
  it("seeds six members on the merchant's own domain (five active, one invite)", async () => {
    const { rows } = await listMembers(demo, ALL);
  ...
  it("invited members have no join date or last-active, and a join date is set on reactivation", async () => {
    const { rows } = await listMembers(demo, { status: "INVITED", pageSize: 100 });
    ...
    const reactivated = await reactivateMember(demo, invite.id);
  ...
  it("inviteMember adds an INVITED member that the Pending Invites view can see", async () => {
    const pendingBefore = (await listMembers(demo, { status: "INVITED", pageSize: 100 })).total;
    const member = await inviteMember(demo, {
      name: "Anna Wijaya",
    ...
    const pending = await listMembers(demo, { status: "INVITED", pageSize: 100 });
  ...
  it("changeMemberRole, deactivate/reactivate, resend and revoke all take effect", async () => {
    ...
    const asAdmin = await changeMemberRole(demo, member.id, "ADMIN");
    expect(asAdmin?.role).toBe("ADMIN");

    const resent = await resendInvite(demo, member.id);
    expect(resent?.invitedAt).not.toBeNull();

    const revoked = await revokeInvite(demo, member.id);
    expect(revoked).toBe(true);
    expect(await getMember(demo, member.id)).toBeNull();
    // revoking a non-invite is a no-op
    const { rows } = await listMembers(demo, ALL);
    const active = rows.find((m) => m.status === "ACTIVE");
    expect(await revokeInvite(demo, active!.id)).toBe(false);
    expect(await getMember(demo, active!.id)).not.toBeNull();

    // deactivate + reactivate round trip
    const deactivated = await deactivateMember(demo, active!.id);
    expect(deactivated?.status).toBe("DEACTIVATED");
    const back = await reactivateMember(demo, active!.id);
    expect(back?.status).toBe("ACTIVE");
  });

  it("the roles catalog reports four roles with derived active member counts", async () => {
    const catalog = await roleCatalog(demo);
    ...
    const { total } = await listMembers(demo, ALL);
