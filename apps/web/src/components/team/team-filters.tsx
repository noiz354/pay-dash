...
/**
 * Team toolbar (ADR-0022) — the prototype's unwired search box and
 * decorative "Filter" dropdown, now URL-driven (`q`, `role`) like the other
 * directories.
 */
export function TeamFilters({ resultCount }: { resultCount: number }) {
  ...
  const hasFilters = role !== "ALL" || urlQuery.length > 0;
    ...
    <div className="flex flex-col items-center justify-between gap-4 border-b border-[var(--border-subtle)] bg-[var(--surface-container-lowest)] px-4 py-3 sm:flex-row">
      <div className="flex w-full flex-wrap items-center gap-2 sm:w-auto">
        ...
        <NativeSelect
          value={role}
          onChange={(e) =>
            push((p) => (e.target.value === "ALL" ? p.delete("role") : p.set("role", e.target.value)))
          }
        ...
        {hasFilters ? (
          <button
            ...
            onClick={() => push((p) => void (p.delete("q"), p.delete("role")))}
          >
            Clear ({resultCount})
          </button>
        ) : (
          <span className="body-sm text-[var(--on-surface-variant)]">{resultCount} members</span>
        )}
      </div>
    </div>
  );
