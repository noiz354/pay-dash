...
import {
  ...
  WEBHOOK_TYPE_FILTERS,
} from "@/lib/webhook-status";

// Webhook log toolbar. All filter state lives in the URL (shareable,
// back-button friendly, server-rendered) — the same contract as the balance,
// ledger, batch and link toolbars. Native selects so the view is scriptable
// in e2e.
export function WebhookFilters({ resultCount }: { resultCount: number }) {
  ...
  const hasFilters = status !== "all" || type !== "all" || (searchParams.get("q") ?? "").length > 0;
        ...
        >
          ...
          {WEBHOOK_STATUSES.map((s) => (
            <NativeSelectOption key={s} value={s}>
              {WEBHOOK_STATUS_LABELS[s]}
            </NativeSelectOption>
          ))}
        </NativeSelect>

        <NativeSelect
          aria-label="Filter callbacks by event type"
          value={type}
          onChange={(e) => push((p) => (e.target.value === "all" ? p.delete("type") : p.set("type", e.target.value)))}
          className={selectClass}
        >
          <NativeSelectOption value="all">Event: All</NativeSelectOption>
          {WEBHOOK_TYPE_FILTERS.map((t) => (
            <NativeSelectOption key={t} value={t}>
              {t === "unknown" ? "Unhandled types" : t}
            </NativeSelectOption>
          ))}
        </NativeSelect>
