...
// Balance history toolbar. All filter state lives in the URL (shareable,
// back-button friendly, server-rendered) — the same pattern as the ledger and
// batch toolbars. Native selects so the view is scriptable in e2e.
export function MovementsFilters({ resultCount }: { resultCount: number }) {
  ...
  const hasFilters =
    type !== "all" || status !== "all" || range !== "all" || (searchParams.get("q") ?? "").length > 0;
    ...
    <div className="flex flex-wrap gap-3 justify-between items-center p-4 border-b border-[var(--border-subtle)] bg-[var(--surface)]">
      <div className="flex flex-wrap gap-2 items-center">
        <NativeSelect
          ...
          value={type}
          onChange={(e) => push((p) => (e.target.value === "all" ? p.delete("type") : p.set("type", e.target.value)))}
        ...
        >
          ...
          {MOVEMENT_STATUSES.map((s) => (
            <NativeSelectOption key={s} value={s}>
              {MOVEMENT_STATUS_LABELS[s]}
            </NativeSelectOption>
          ))}
        </NativeSelect>

        <NativeSelect
          aria-label="Filter movements by date"
          value={range}
          onChange={(e) => push((p) => (e.target.value === "all" ? p.delete("range") : p.set("range", e.target.value)))}
          className={selectClass}
        >
          <NativeSelectOption value="all">Date: All time</NativeSelectOption>
          <NativeSelectOption value="7d">Date: Last 7 days</NativeSelectOption>
          <NativeSelectOption value="30d">Date: Last 30 days</NativeSelectOption>
          <NativeSelectOption value="90d">Date: Last 90 days</NativeSelectOption>
        </NativeSelect>
      ...
      <div className="flex items-center gap-3">
        <span className="body-sm text-[var(--on-surface-variant)] flex items-center gap-2" aria-live="polite">
          ...
          <span className="data-mono text-xs">{resultCount.toLocaleString("en-US")} movements</span>
        </span>
